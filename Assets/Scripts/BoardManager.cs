...
public class BoardManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (sharedInstance == null) //Buenas practicas
            sharedInstance = this;
        else
        {
            Destroy(gameObject);
    ...
    private void CreateInitialBoard(Vector2 offset)
    {
        ...
        for (int x = 0; x < xSize; x++)
        {
            ...
            {
                GameObject newSquare = Instantiate(square, new Vector3(startX + 0.5f + (offset.x * x),
                                                                          startY + 0.5f + (offset.y * y),
                                                                          0), square.transform.rotation);
                newSquare.transform.parent = squareParent.transform;

                GameObject newCandy = Instantiate(
                currentCandy,
                    new Vector3(startX + 0.5f + (offset.x * x),
                                startY + 0.5f + (offset.y * y),
                                0),
